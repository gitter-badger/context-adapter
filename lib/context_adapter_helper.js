...
function getOperationDescriptor(request, callback) {
  ...
  operationDescriptor.externalButtonId = '<external-button-id>';
...
function getResult(err, data) {
  if (err) {
    ...
    if (data.body) {
      return data.body.result || 'Default_result_when_no_result_property_in_Third_Party_response';
    } else {
      return 'The_Third_Party_responded_with_an_unknown_response_format'
    }
  }
}
...
function sendThirdPartyRequest(updateContextRequest, callback) {
  ...
  caLogger.info(
    ...
    'Sending third party request: ' +
    caConfig.THIRD_PARTY_METHOD + ' ' + caConfig.THIRD_PARTY_ENDPOINT +
    ' with payload: ' + JSON.stringify(updateContextRequest.contextAdapter.operationDescriptor)
  );
  request({
    method: caConfig.THIRD_PARTY_METHOD,
    ...
    json: true,
    body: updateContextRequest.contextAdapter.operationDescriptor
  }, callback);
}
...
function notifyOperationCompleted(request, response, callback) {
  ...
  if (request.contextAdapter.operationDescriptor.interactionType !==
    caConfig.BUTTON_ENTITY.CA_OPERATION_INTERACTION_TYPE.ASYNC) {
